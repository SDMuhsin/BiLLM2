...
@torch.no_grad()
def opt_eval(model, testenc, dev,save_title="UNNAMED_OPT"):
    print('Evaluating ...')
...
@torch.no_grad()
def opt_eval_wrong(model, testenc, dev, dataset: str, log_wandb: bool = False, save_title = 'UNNAMED_OPT'):
...
if __name__ == '__main__':
    ...
    if (args.nearest):
        save_title = f"{args.model}_{args.dataset}_rtn{args.wbits}_128_hessian"
    else:
        save_title = f"{args.model}_{args.dataset}_gptq{args.wbits}_128_hessian"
    ...
    for dataset in [args.dataset]: 
        ...
        print(dataset)
        #opt_eval(model, testloader, DEV, dataset, save_title= save_title)
        opt_eval(model, testloader, DEV,save_title=save_title)
